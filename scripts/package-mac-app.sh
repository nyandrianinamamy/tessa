...
# Build and bundle Tessa into a minimal .app we can open.
# Outputs to dist/Tessa.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Tessa.app"
...
/usr/libexec/PlistBuddy -c "Set :TessaBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :TessaGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Tessa"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Tessa"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Tessa"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Tessa" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/OpenClaw/Resources/Tessa.icns" "$APP_ROOT/Contents/Resources/Tessa.icns"
...
echo "⏹  Stopping any running Tessa"
killall -q Tessa 2>/dev/null || true
killall -q Moltbot 2>/dev/null || true  # Legacy name
