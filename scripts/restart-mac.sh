...
APP_PROCESS_PATTERN="Tessa.app/Contents/MacOS/Tessa"
LEGACY_APP_PROCESS_PATTERN="Moltbot.app/Contents/MacOS/Moltbot"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Tessa"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Tessa"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Tessa"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/bot.molt.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/tessa-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${CLAWDBOT_RESTART_LOG:-/tmp/tessa-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${CLAWDBOT_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.tessa/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log "  node tessa.mjs daemon install --force --runtime node"
      log "  node tessa.mjs daemon restart"
      log ""
...
kill_all_tessa() {
  for _ in {1..10}; do
    ...
    pkill -x "Tessa" 2>/dev/null || true
    pkill -x "Moltbot" 2>/dev/null || true  # Legacy
...
log "==> Killing existing Tessa instances"
kill_all_tessa
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Tessa"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Tessa.app" ]]; then
    APP_BUNDLE="/Applications/Tessa.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Tessa.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Tessa.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Tessa.app missing Sparkle after packaging"
    fi
    return 0
  fi

  # Fallback to legacy Moltbot.app
  ...
  fail "App bundle not found. Set CLAWDBOT_APP_BUNDLE to your installed Tessa.app or Moltbot.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node tessa.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node tessa.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".tessa", "tessa.json"), "utf8");
