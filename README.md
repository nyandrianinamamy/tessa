# 🦞 Tessa — Personal AI Assistant

<p align="center">
  <img src="https://raw.githubusercontent.com/moltbot/moltbot/main/docs/whatsapp-clawd.jpg" alt="Tessa" width="400">
</p>
...
<p align="center">
  ...
  <a href="https://deepwiki.com/moltbot/moltbot"><img src="https://img.shields.io/badge/DeepWiki-tessa-111111?style=for-the-badge" alt="DeepWiki"></a>
  <a href="https://discord.gg/clawd"><img src="https://img.shields.io/discord/1456350064065904867?label=Discord&logo=discord&logoColor=white&color=5865F2&style=for-the-badge" alt="Discord"></a>
  <a href="LICENSE"><img src="https://img.shields.io/badge/License-MIT-blue.svg?style=for-the-badge" alt="MIT License"></a>
</p>

**Tessa** is a *personal AI assistant* you run on your own devices.
...
Preferred setup: run the onboarding wizard (`tessa onboard`). It walks through gateway, workspace, channels, and skills. The CLI wizard is the recommended path and works on **macOS, Linux, and Windows (via WSL2; strongly recommended)**.
...
```bash
npm install -g tessa@latest
# or: pnpm add -g tessa@latest

tessa onboard --install-daemon
```

The wizard installs the Gateway daemon (launchd/systemd user service) so it stays running.
Legacy note: `moltbot` and `clawdbot` commands remain available as compatibility aliases.
...
tessa gateway --port 18789 --verbose

# Send a message
tessa message send --to +1234567890 --message "Hello from Tessa"

# Talk to the assistant (optionally deliver back to any connected channel: WhatsApp/Telegram/Slack/Discord/Google Chat/Signal/iMessage/BlueBubbles/Microsoft Teams/Matrix/Zalo/Zalo Personal/WebChat)
tessa agent --message "Ship checklist" --thinking high
```

Upgrading? [Updating guide](https://docs.molt.bot/install/updating) (and run `tessa doctor`).
...
Switch channels (git + npm): `tessa update --channel stable|beta|dev`.
...
pnpm tessa onboard --install-daemon

# Dev loop (auto-reload on TS changes)
pnpm gateway:watch
```

Note: `pnpm tessa ...` runs TypeScript directly (via `tsx`). `pnpm build` produces `dist/` for running via Node / the packaged `moltbot` binary.

## Security defaults (DM access)

Tessa connects to real messaging surfaces. Treat inbound DMs as **untrusted input**.

Full security guide: [Security](https://docs.molt.bot/gateway/security)

Default behavior on Telegram/WhatsApp/Signal/iMessage/Microsoft Teams/Discord/Google Chat/Slack:
- **DM pairing** (`dmPolicy="pairing"` / `channels.discord.dm.policy="pairing"` / `channels.slack.dm.policy="pairing"`): unknown senders receive a short pairing code and the bot does not process their message.
- Approve with: `tessa pairing approve <channel> <code>` (then the sender is added to a local allowlist store).
- Public inbound DMs require an explicit opt-in: set `dmPolicy="open"` and include `"*"` in the channel allowlist (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`).

Run `tessa doctor` to surface risky/misconfigured DM policies.
...
### Tools + automation
- [Browser control](https://docs.molt.bot/tools/browser): dedicated tessa Chrome/Chromium, snapshots, actions, uploads, profiles.
...
Tessa can auto-configure Tailscale **Serve** (tailnet-only) or **Funnel** (public) while the Gateway stays bound to loopback. Configure `gateway.tailscale.mode`:
...
Notes:
- `gateway.bind` must stay `loopback` when Serve/Funnel is enabled (Tessa enforces this).
...
Minimal `~/.tessa/tessa.json` (model + defaults):
...
- Link the device: `pnpm tessa channels login` (stores creds in `~/.tessa/credentials`).
...
Tessa was built for **Molty**, a space lobster AI assistant. 🦞
