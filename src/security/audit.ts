...
function collectBrowserControlFindings(cfg: OpenClawConfig): SecurityAuditFinding[] {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("tessa security audit --deep")}".`,
    });
    return findings;
  }
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && deep.gateway.ok === false) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("tessa status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("tessa security audit --deep")}".`,
    });
  }
