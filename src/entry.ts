...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.TESSA_NO_RESPAWN || process.env.CLAWDBOT_NO_RESPAWN))
    return false;
  if (
    isTruthyEnvValue(
      process.env.TESSA_NODE_OPTIONS_READY || process.env.CLAWDBOT_NODE_OPTIONS_READY,
    )
  )
    return false;
  ...
  process.env.TESSA_NODE_OPTIONS_READY = "1";
  process.env.CLAWDBOT_NODE_OPTIONS_READY = "1"; // Legacy compatibility
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[tessa] ${parsed.error}`);
    process.exit(2);
  }
