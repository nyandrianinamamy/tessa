...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Tessa status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("tessa security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("tessa security audit --deep")}`));
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("tessa status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("tessa logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("tessa status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("tessa gateway probe")}`);
  }
}
