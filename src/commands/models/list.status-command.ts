...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("tessa models auth setup-token")}\` or \`${formatCliCommand("tessa configure")}\`.`
          : `Run \`${formatCliCommand("tessa configure")}\` or set an API key env var.`;
