...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.tessa${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.TESSA_PROFILE = profile;

  const stateDir = env.TESSA_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.TESSA_STATE_DIR?.trim()) env.TESSA_STATE_DIR = stateDir;

  if (!env.TESSA_CONFIG_PATH?.trim()) {
    env.TESSA_CONFIG_PATH = path.join(stateDir, "tessa.json");
  }

  if (profile === "dev" && !env.TESSA_GATEWAY_PORT?.trim()) {
    env.TESSA_GATEWAY_PORT = "19001";
  }
}
