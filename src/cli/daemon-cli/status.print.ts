...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean }) {
  ...
  if (service.configAudit?.issues.length) {
    ...
    defaultRuntime.error(
      warnText(
        `Recommendation: run "${formatCliCommand("tessa doctor")}" (or "${formatCliCommand("tessa doctor --repair")}").`,
      ),
    );
  }
  ...
  if (status.config) {
    ...
    if (status.config.mismatch) {
      ...
      defaultRuntime.error(
        errorText(
          `Fix: rerun \`${formatCliCommand("tessa gateway install --force")}\` from the same --profile / OPENCLAW_STATE_DIR you expect.`,
        ),
      );
    }
    spacer();
  ...
  if (service.runtime?.cachedLabel) {
    ...
    defaultRuntime.error(errorText(`Then reinstall: ${formatCliCommand("tessa gateway install")}`));
    spacer();
  }
  ...
  defaultRuntime.log(`${label("Troubles:")} run ${formatCliCommand("tessa status")}`);
