...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("tessa node install"), formatCliCommand("tessa node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("tessa node install --force")}`);
    }
    return;
  }
